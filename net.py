...
class TRACKNET:
    ...
    def build(self):
        ...
        with tf.variable_scope("fc_two_nets"):
            ...
            _activation_summary(x)
            x = tf.layers.dense(x, 4096, name='fc3', activation=tf.nn.relu, kernel_regularizer=self.wreg , bias_regularizer=self.breg)
            _activation_summary(x)
            self.fc_output = tf.layers.dense(x,    4, name='fc4', activation=None, kernel_regularizer=self.wreg , bias_regularizer=self.breg)
