...
class DataPrep:
    ...
    def __init__(self, data_dir, batch_size):
        ...
        #-----------------------------------------------------------------------
        try:
            ftrain = open(data_dir + '/trainSet.txt', "r")
            ftest = open(data_dir + '/testSet.txt', "r")

        except (FileNotFoundError, IOError) as e:
            raise RuntimeError(str(e))

        [train_target, train_search, train_box] = load_training_set(ftrain)
        [test_target, test_search, test_box] = load_training_set(ftest)
        ...
        self.batch_test_queue = next_batch(input_test_queue, 1)
